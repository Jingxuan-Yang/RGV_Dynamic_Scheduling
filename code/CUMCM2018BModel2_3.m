...
%   分组如何遍历，以及遍历完毕后输出最优解。而（4,4）分组和（5,3）分组遍历
%   在程序结束后的注释给出。

%   通过计算得到，第一道工序CNC序号为1,3,4,6,8；第二道工序CNC序号为1,3,5,7
%   （最优解不唯一）
...
for divgroup = 1: 56
    ...
    starttime1 = zeros(50, CNCnum);%   每台机器第一道工序上料所对应时间
    starttime2 = zeros(50, CNCnum);%   每台机器第二道工序上料所对应时间
    endtime1 = zeros(50, CNCnum);  %   每台机器第一道工序下料所对应时间
    endtime2 = zeros(50, CNCnum);  %   每台机器第二道工序下料所对应时间
    %   总时间不超过8小时
    while t < Twork
...
%             switch minPos
%                 %   若操作对象为第一道工序CNC
%                 case {firstgroup(divgroup, 1), firstgroup(divgroup, 2), firstgroup(divgroup, 3), firstgroup(divgroup, 4)}
%                     %   若操作对象未开始工作，进行上料处理并计数
%                     if CNCw(minPos) == 0
%                         t = t + Trgvw(minPos);
%                         Tcncw(CNCw == 1) = Tcncw(CNCw == 1) - Trgvw(minPos);
...
%                     end
%                 %   若操作对象为第二道工序CNC
%                 case {secondgroup(divgroup, 1), secondgroup(divgroup, 2), secondgroup(divgroup, 3), secondgroup(divgroup, 4)}
%                     %   若操作对象未开始工作，上料并计数
%                     if CNCw(minPos) == 0
%                         t = t + Trgvw(minPos);
%                         Tcncw(CNCw == 1) = Tcncw(CNCw == 1) - Trgvw(minPos);
...
%             switch minPos
%                 %   若操作对象为第一道工序CNC
%                 case {firstgroup3(divgroup, 1), firstgroup3(divgroup, 2), firstgroup3(divgroup, 3)}
%                     %   若操作对象未开始工作，进行上料处理并计数
%                     if CNCw(minPos) == 0
%                         t = t + Trgvw(minPos);
%                         Tcncw(CNCw == 1) = Tcncw(CNCw == 1) - Trgvw(minPos);
...
%                     end
%                 %   若操作对象为第二道工序CNC
%                 case {secondgroup(divgroup, 1), secondgroup(divgroup, 2), secondgroup(divgroup, 3), secondgroup(divgroup, 4), secondgroup(divgroup, 5)}
%                     %   若操作对象未开始工作，上料并计数
%                     if CNCw(minPos) == 0
%                         t = t + Trgvw(minPos);
%                         Tcncw(CNCw == 1) = Tcncw(CNCw == 1) - Trgvw(minPos);
